...
const suggestions = [
  { label: 'calc 12*4+1', hint: 'inline calculator' },
  { label: 'Notes', hint: 'quick-capture, saved locally' },
  { label: 'Todos', hint: 'a tiny task list' },
  { label: 'Search GitHub', hint: 'live repo search' },
  { label: 'Change Theme', hint: 'dark / light / system' },
  { label: 'Help & Shortcuts', hint: 'full command reference' },
]
...
export default function Home() {
  ...
  return (
    <div className="min-h-[calc(100vh-57px)] flex flex-col items-center justify-center gap-8 text-center px-4">
      ...
      <button
        onClick={toggle}
        className="group flex items-center gap-3 px-5 py-3 rounded-xl bg-panel border border-border hover:border-accent/50 shadow-lg transition"
      >
      ...
      <div className="flex flex-wrap justify-center gap-2 max-w-lg">
        {suggestions.map((s) => (
          <span
            key={s.label}
            className="text-xs px-3 py-1.5 rounded-full bg-panel border border-border text-fg-500"
            title={s.hint}
          >
            {s.label}
          </span>
